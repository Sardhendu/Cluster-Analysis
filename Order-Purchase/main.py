...
from crt_dataset_models import build_dataset_jaccard, find_opportune_centroids, cluster_k_means, build_the_cluster, cal_cost_function_distance
...
#=================== (WITH A SUBSET OF ATTRIBUTES) INITIALIZE THE PARAMETERS ==============================#
...
if inner_outer_cluster==0:
    data_train = pd.read_csv(conf['dataset1'])#, index_col='Date', parse_dates=True)
else:
    data_train = pd.read_csv(conf['dataset1'])
...
#==============================================================================
#  FIND THE OPPORTUNE CENTROIDS -- Not used Now
#==============================================================================
...
if inner_outer_cluster==0:
    for cluster_number in range(0,len(cluster)):
        ...
        np.savetxt(conf['clusters']+"clusters_%i.csv"%cluster_number, a,delimiter=",", fmt="%s")
else:
    for cluster_number in range(0,len(cluster)):
        a=np.asarray(cluster[cluster_number])
        np.savetxt(conf['inner_clusters']+"clusters_%i.csv"%cluster_number, a,delimiter=",", fmt="%s")    
...
print 'Done - The algorithm finally ceases'
